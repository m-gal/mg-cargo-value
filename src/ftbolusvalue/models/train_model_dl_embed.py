...
# * get stats for description's length in term of words
# prc = [0.5, 0.9, 0.95, 0.99, 0.995, 0.999, 0.9999]
# df_train["description"].apply(lambda x: np.mean(len(x.split()))).describe(percentiles=prc)
TEXT_TO_EMBEDDED = {
    "description": {
        "use_embedding": True,
        ...
        "output_sequence_length": 200,  # Only valid in "int" mode
        ...
        # * Related to Embedding layer:
        "output_dim": 512,  # None or Int
        "mask_zero": True,  # Bool.
        "input_length": None,  # None or Int
        "flat_or_pool": "flat",  # ("avgpool", "maxpool", "flat"). To use Flatten or Global..Pooling1D
    }
}
...
# ds_train_gen = DataGenerator(ds_train, y_train_true, MODEL_PARAMS["_batch_size"])
# ds_val_gen = DataGenerator(ds_val, y_val_true, MODEL_PARAMS["_batch_size"])
...
# %% Load model ----------------------------------------------------------------
# MODEL_PATH = Path("../tracks/20231207-131418/model")
